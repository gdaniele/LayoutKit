...
class NestedCollectionViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        layout(collectionView.frame.width, synchronous: false)
    }

    private func layout(_ width: CGFloat, synchronous: Bool) {
        collectionView.layoutAdapter.reload(width, synchronous: synchronous, layoutProvider: {
    ...
    override func viewWillTransition(to size: CGSize, with coordinator: UIViewControllerTransitionCoordinator) {
        ...
        layout(size.width, synchronous: true)
    }
    ...
    private static func makeLayout() -> LazyMapCollection<(CountableRange<Int>), Layout> {
        return (1..<100).lazy.map { index in
            let layout = sectionLayout(index, count: index)
    ...
    // Static to avoid capturing a reference to self
    private static func sectionLayout(_ sectionIndex: Int, count: Int) -> [Section<LazyMapRandomAccessCollection<CountableRange<Int>, Layout>>] {
